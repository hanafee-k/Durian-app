...
const TopNav = () => {
  ...
  return (
    <nav className="hidden lg:block w-full bg-white/90 backdrop-blur-2xl border-b border-slate-200/60 sticky top-0 z-50 transition-all duration-300">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">

        {/* โลโก้ (บังคับฟอนต์ Prompt) */}
        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate('/')}>
          <div className="w-12 h-12 bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-[18px] flex items-center justify-center shadow-lg shadow-emerald-500/20 group-hover:scale-105 group-hover:rotate-3 transition-transform duration-300">
        ...
        {/* เมนูตรงกลาง (บังคับฟอนต์ Kanit) */}
        <div className="flex items-center p-1.5 bg-slate-50/80 rounded-2xl border border-slate-200/50 backdrop-blur-sm">
          {TABS.map(({ id, label, Icon }) => {
            ...
            return (
              <button
                key={id}
                onClick={() => navigate(id)}
                className={`px-5 py-2.5 rounded-[12px] flex items-center gap-2.5 text-sm font-semibold font-['Kanit'] transition-all duration-300 ease-out ${isActive
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="bg-emerald-600 text-white px-6 py-3 rounded-2xl font-bold font-['Kanit'] text-sm shadow-lg shadow-emerald-600/20 hover:bg-emerald-500 hover:-translate-y-0.5 active:scale-95 transition-all flex items-center gap-2"
          >
